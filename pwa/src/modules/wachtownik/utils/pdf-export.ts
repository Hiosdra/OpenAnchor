...
/**
 * Exports a blank watch schedule template PDF — same table structure but
 * without crew names filled in. Useful for printing an empty form to fill
 * by hand.
 */
export function exportBlankScheduleToPDF(
  schedule: DaySchedule[],
  startDate: string,
  slots: WatchSlot[],
  userLocale: Locale,
  ...
  doc.setFontSize(16);
  doc.text('Wachtownik — szablon grafiku', 148, 15, { align: 'center' });
  ...
  const numSlots = slotsSource.length || 6;
  const tableWidth = 277 - 20;
  const dateColWidth = 22;
  const slotColWidth = Math.max(25, (tableWidth - dateColWidth) / numSlots);
  void slotColWidth;
  ...
  const filenameSafeStartDate = startDateObj.toISOString().split('T')[0];
  const filenameSafeEndDate = endDateObj.toISOString().split('T')[0];
  const filename = `szablon_wacht_${filenameSafeStartDate}_${filenameSafeEndDate}.pdf`;
  doc.save(filename);
}
