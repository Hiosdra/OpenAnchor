...
export function GanttChart({ isNightMode, userLocale, schedule, startDate }: GanttChartProps) {
  const allCrewMembers = [
    ...new Set(
      schedule.flatMap((day) => day.slots.flatMap((slot) => slot.assigned.map((p) => p.name))),
    ),
  ];
  ...
  return (
    ...
    >
      ...
      {schedule.map((daySchedule, dayIndex) => {
        ...
        return (
          <div key={dayIndex} className="mb-8 last:mb-0">
            {/* Day header */}
            <div
              className={`mb-3 pb-2 border-b-2 ${isNightMode ? 'border-red-900' : 'border-slate-300'}`}
            >
              <h3 className={`text-lg font-bold ${isNightMode ? 'text-red-400' : 'text-sky-800'}`}>
                {t('common.day', userLocale)} {daySchedule.day}
                <span
                  className={`ml-2 text-sm font-normal ${isNightMode ? 'text-red-700' : 'text-slate-500'}`}
                >
                  (
                  {dayDate.toLocaleDateString(userLocale, {
                    weekday: 'short',
                    day: '2-digit',
                    month: '2-digit',
                    year: 'numeric',
            ...
            <div className="overflow-x-auto">
              <div className="min-w-[800px]">
                ...
                <div className="flex mb-4">
                  <div
                    className={`w-32 font-bold text-sm ${isNightMode ? 'text-red-700' : 'text-slate-600'}`}
                  >
                    {t('label.crewMember', userLocale)}
                  </div>
                  <div
                    className="flex-1 flex border-l border-r"
                    style={{ borderColor: isNightMode ? '#7f1d1d' : '#cbd5e1' }}
                  >
                    {Array.from({ length: 24 }, (_, hour) => (
                      <div
                        key={hour}
                        className={`flex-1 text-center text-xs border-r ${isNightMode ? 'border-red-900/30 text-red-800' : 'border-slate-200 text-slate-500'}`}
                      >
                ...
                {allCrewMembers.map((crewName) => (
                  <div key={crewName} className="mb-2">
                    <div className="flex items-stretch">
                      <div
                        className={`w-32 font-medium text-sm flex items-center ${isNightMode ? 'text-red-400' : 'text-slate-700'}`}
                      >
                        {crewName}
                      </div>
                      <div
                        className="flex-1 relative h-8 border-l border-r"
                        style={{ borderColor: isNightMode ? '#7f1d1d' : '#cbd5e1' }}
                      >
                        {/* 24-hour grid lines */}
                        ...
                        {daySchedule.slots.map((slot, idx) => {
                          ...
                          const [endH, endM] =
                            slot.end === '24:00' ? [24, 0] : slot.end.split(':').map(Number);
                          ...
                          const colors = [
                            ...
                            {
                              bg: isNightMode ? 'bg-green-900' : 'bg-green-500',
                              text: 'text-white',
                            },
                            {
                              bg: isNightMode ? 'bg-purple-900' : 'bg-purple-500',
                              text: 'text-white',
                            },
                            {
                              bg: isNightMode ? 'bg-orange-900' : 'bg-orange-500',
                              text: 'text-white',
                            },
      ...
      <div
        className={`mt-6 p-4 rounded-lg ${isNightMode ? 'bg-black border border-red-900' : 'bg-slate-50 border border-slate-200'}`}
      >
