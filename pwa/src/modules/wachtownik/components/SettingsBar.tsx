...
export function SettingsBar({
  ...
  return (
    <header
      className={`oa-header print:bg-white print:text-black print:shadow-none print:p-0 print:mb-6 ${isNightMode ? 'bg-zinc-950/90 border-b border-red-900/20' : ''}`}
    >
      <a
        href="../../index.html"
        title={t('title.backToMenu', userLocale)}
        aria-label={t('aria.backToMenu', userLocale)}
        className="oa-back-btn print:hidden"
      >
        <svg
          width="16"
          height="16"
      ...
      <div className="flex flex-wrap justify-center gap-1.5 sm:gap-2 print:hidden">
        ...
        <button
          ...
          aria-label={
            isNightMode
              ? t('aria.disableNightMode', userLocale)
              : t('aria.toggleNightMode', userLocale)
          }
        ...
        {!isReadOnly && (
          <Dropdown
            ...
            buttonClassName={
              isNightMode
                ? 'border-red-800 hover:bg-red-950 text-red-500'
