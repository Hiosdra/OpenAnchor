...
export function WatchSlotsPanel({
  ...
  return (
    <div
      className={`lg:col-span-8 p-4 sm:p-5 rounded-xl shadow-sm border ${isNightMode ? 'bg-zinc-950 border-red-900' : 'bg-white border-slate-200 text-slate-800'}`}
    >
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 sm:mb-6 gap-2">
        <div
          className={`flex items-center space-x-2 ${isNightMode ? 'text-red-500' : 'text-sky-900'}`}
        >
          <Icon name="Clock" className="w-5 h-5" />{' '}
          <h2 className="text-base sm:text-lg font-semibold">
            {t('heading.watchSystem', userLocale)}
          </h2>
        </div>
        <button
          onClick={applyDogWatches}
          className={`px-3 py-1.5 rounded-md text-xs font-medium transition border ${isNightMode ? 'bg-red-950 border-red-800 text-red-500 hover:bg-red-900' : 'bg-sky-100 text-sky-800 border-sky-200 hover:bg-sky-200'}`}
        >
      ...
      {recommendations.length > 0 && (
        <div
          className={`mb-6 p-4 rounded-lg border ${isNightMode ? 'bg-black border-red-900' : 'bg-emerald-50 border-emerald-200'}`}
        >
          <h3
            className={`text-sm font-semibold mb-3 flex items-center space-x-2 ${isNightMode ? 'text-red-400' : 'text-emerald-800'}`}
          >
              ...
              <button
                ...
                className={`w-full text-left p-3 rounded-md border transition-all ${
                  idx === 0
                    ...
                    : isNightMode
                      ? 'bg-zinc-900 border-red-800 hover:bg-zinc-800'
              ...
              >
                <div className="flex items-start justify-between pointer-events-none">
                  <div className="flex-1">
                    <div
                      className={`text-sm font-semibold ${isNightMode ? 'text-red-400' : 'text-emerald-700'}`}
                    >
                      {idx === 0 && '⭐ '}
                      {t(rec.template.nameKey, userLocale)}
                    </div>
                    <div
                      className={`text-xs mt-1 ${isNightMode ? 'text-red-700' : 'text-emerald-600'}`}
                    >
                      {rec.reason}
                    </div>
                  </div>
                  {idx === 0 && (
                    <span
                      className={`text-xs px-2 py-1 rounded-full ml-2 ${isNightMode ? 'bg-red-900 text-black' : 'bg-emerald-200 text-emerald-800'}`}
                    >
      ...
      <div
        className={`mb-6 p-4 rounded-lg border ${isNightMode ? 'bg-black border-red-900' : 'bg-slate-50 border-slate-200'}`}
      >
        <h3
          className={`text-sm font-semibold mb-3 flex items-center space-x-2 ${isNightMode ? 'text-red-400' : 'text-sky-800'}`}
        >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2">
          {Object.entries(WATCH_TEMPLATES).map(([key, template]) => (
            ...
            >
              <div
                className={`text-sm font-semibold mb-1 ${isNightMode ? 'text-red-400' : 'text-sky-700'}`}
              >
          ...
          return (
            <div
              className={`mb-6 p-4 rounded-lg border ${
                coverage.hasFull24h
                  ...
                  : isNightMode
                    ? 'bg-orange-950/20 border-orange-900'
            ...
            >
              <div className="flex items-center justify-between mb-3">
                ...
                >
                  <Icon
                    name={coverage.hasFull24h ? 'CheckCircle' : 'XCircle'}
                    className="w-4 h-4"
                  />
                  <span>
                    {coverage.hasFull24h
                      ? t('msg.coverage24h', userLocale)
                      : t('msg.coverageGap', userLocale)}
                  </span>
                </h3>
                <span
              ...
              <div className="mb-3">
                <div
                  className={`h-8 rounded overflow-hidden flex ${isNightMode ? 'bg-black' : 'bg-slate-200'}`}
                >
              ...
              {coverage.gaps.length > 0 && (
                <div className={`text-xs ${isNightMode ? 'text-orange-600' : 'text-orange-700'}`}>
                  ...
                  {coverage.gaps.map((gap, i) => (
                    <span key={i}>
                      {gap.start}-{gap.end} ({Math.round((gap.minutes / 60) * 10) / 10}h)
      ...
      <div className="sm:hidden space-y-3 mb-4" id="slots-configuration">
        {slots.map((slot) => (
          <div
            key={slot.id}
            className={`p-4 rounded-lg border ${isNightMode ? 'bg-zinc-900 border-red-800' : 'bg-white border-slate-300'}`}
          >
            <div className="space-y-3">
              <div>
                <label
                  className={`block text-xs font-medium mb-1 ${isNightMode ? 'text-red-600' : 'text-slate-600'}`}
                >
      ...
      <div className="hidden sm:block overflow-x-auto" id="slots-configuration-desktop">
        <table className="w-full text-sm text-left">
          <thead
            className={`text-xs uppercase ${isNightMode ? 'bg-zinc-900 text-red-700' : 'bg-slate-50 text-slate-500'}`}
          >
            <tr>
              <th className="px-4 py-3 rounded-tl-lg">{t('label.start', userLocale)}</th>
              <th className="px-4 py-3">{t('label.end', userLocale)}</th>
              <th className="px-4 py-3">{t('label.requiredCrew', userLocale)}</th>
              <th className="px-4 py-3 rounded-tr-lg text-right">
                {t('label.action', userLocale)}
              </th>
            </tr>
          </thead>
          <tbody>
            {slots.map((slot) => (
              <tr
                key={slot.id}
                className={`border-b last:border-0 ${isNightMode ? 'border-red-900/50' : 'border-slate-100'}`}
              >
                <td className="px-4 py-2">
                  <input
                    ...
                    value={slot.start}
                    onChange={(e) => updateSlot(slot.id, 'start', e.target.value)}
                    className={`border p-1 rounded ${isNightMode ? 'bg-black border-red-800 text-red-400' : 'bg-white border-slate-300'}`}
                    style={isNightMode ? { colorScheme: 'dark' } : {}}
                  />
                </td>
                <td className="px-4 py-2">
                ...
                <td className="px-4 py-2">
                  <input
                    ...
                    value={slot.reqCrew}
                    onChange={(e) => updateSlot(slot.id, 'reqCrew', Number(e.target.value))}
                    className={`border p-1 rounded w-16 ${isNightMode ? 'bg-black border-red-800 text-red-400' : 'bg-white border-slate-300'}`}
                  />
                </td>
      ...
      <button
        onClick={addSlot}
        className={`mt-4 flex items-center space-x-2 text-sm font-medium ${isNightMode ? 'text-red-500 hover:text-red-400' : 'text-sky-600 hover:text-sky-800'}`}
      >
