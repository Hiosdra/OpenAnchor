...
interface ModalConfig {
  id: string; titleId?: string; zIndex?: number;
  outerPadding?: string; innerPadding?: string; maxWidth?: string;
  borderClass?: string; shadowClass?: string; extraInnerClass?: string;
  titleI18n?: string; titleDefault?: string; titleSize?: string;
  titleColor?: string; titleMargin?: string;
  icon?: string; iconClass?: string;
  content: string;
}

function createModal(cfg: ModalConfig): string {
  const tId = cfg.titleId ?? cfg.id.replace(/-modal$/, '') + '-title';
  const border = cfg.borderClass ?? 'border border-slate-700';
  const shadow = cfg.shadowClass ? ` ${cfg.shadowClass}` : '';
  const extra = cfg.extraInnerClass ? ` ${cfg.extraInnerClass}` : '';
  const iconHtml = cfg.icon
    ? `<i data-lucide="${cfg.icon}"${cfg.iconClass ? ` class="${cfg.iconClass}"` : ''}></i> `
    : '';
  const title = cfg.titleI18n
    ? `\n        <h3 id="${tId}" class="${cfg.titleSize ?? 'text-xl'} font-bold ${cfg.titleColor ?? 'text-white'} ${cfg.titleMargin ?? 'mb-4'} flex items-center gap-2">${iconHtml}<span data-i18n="${cfg.titleI18n}">${cfg.titleDefault}</span></h3>`
    : '';
  return `
<div id="${cfg.id}" class="modal fixed inset-0 bg-black/80 flex items-center justify-center z-[${cfg.zIndex ?? 2000}] hidden ${cfg.outerPadding ?? 'p-4'}" role="dialog" aria-modal="true" aria-labelledby="${tId}">
    <div class="bg-slate-800 ${cfg.innerPadding ?? 'p-6'} rounded-2xl ${cfg.maxWidth ?? 'max-w-sm'} w-full ${border}${shadow}${extra}">${title}
        ${cfg.content}
    </div>
</div>`;
}
...
export function allModalsHTML(): string {
  return [
    dragWarningModalHTML(), watchSetupModalHTML(), watchAlertModalHTML(),
    calcModalHTML(), sectorModalHTML(), offsetModalHTML(),
    batteryModalHTML(), warningModalHTML(), wsSyncModalHTML(),
    apiKeyModalHTML(), aiModalHTML(), aiSummaryModalHTML(),
    gpsLostModalHTML(), historyModalHTML(), statsModalHTML(),
    replayModalHTML(), weatherModalHTML(), qrScanModalHTML(),
  ].join('\n');
}

function dragWarningModalHTML(): string {
  return createModal({ id: 'drag-warning-modal', zIndex: 4000,
    outerPadding: 'p-3 sm:p-4', innerPadding: 'p-4 sm:p-6',
    borderClass: 'border-2 border-orange-500', shadowClass: 'shadow-[0_0_25px_rgba(249,115,22,0.3)]',
    titleI18n: 'dragTitle', titleDefault: 'Uwaga: Pełzanie?',
    titleSize: 'text-lg sm:text-xl', titleColor: 'text-orange-500', titleMargin: 'mb-2',
    icon: 'alert-circle', content: `<p class="text-slate-300 text-sm mb-4 sm:mb-6 leading-relaxed" data-i18n="dragBody">Algorytm wykrył, że jacht powoli, ale stabilnie oddala się od środka. Sprawdź, czy kotwica nie pełznie!</p>
...
function watchSetupModalHTML(): string {
  return createModal({ id: 'watch-setup-modal', innerPadding: 'p-5',
    extraInnerClass: 'flex flex-col max-h-[90vh]',
    titleI18n: 'watchTitle', titleDefault: 'System Wacht',
    icon: 'clipboard-list', iconClass: 'text-blue-400', content: `<div class="overflow-y-auto flex-grow space-y-4 pr-1">
...
function watchAlertModalHTML(): string {
  return createModal({ id: 'watch-alert-modal', titleId: 'watch-alert-title', zIndex: 5000,
    borderClass: 'border-2 border-blue-500', shadowClass: 'shadow-[0_0_25px_rgba(59,130,246,0.3)]',
    extraInnerClass: 'text-center', content: `<i data-lucide="timer" class="w-16 h-16 text-blue-500 mx-auto mb-4 animate-bounce"></i>
...
function calcModalHTML(): string {
  return createModal({ id: 'calc-modal', icon: 'ruler', iconClass: 'text-blue-400',
    titleI18n: 'calcTitle', titleDefault: 'Kalkulator', content: `<div class="grid grid-cols-2 gap-4 mb-4">
...
function sectorModalHTML(): string {
  return createModal({ id: 'sector-modal', icon: 'pie-chart', iconClass: 'text-blue-400',
    titleI18n: 'sectorTitle', titleDefault: 'Sektor', content: `<label class="flex items-center gap-3 bg-slate-700 p-3 rounded-xl mb-4 border border-slate-600 cursor-pointer"><input type="checkbox" id="sector-enable" class="w-5 h-5 accent-blue-500 rounded"><span class="text-white font-medium" data-i18n="sectorEnable">Aktywuj Sektor</span></label>
...
function offsetModalHTML(): string {
  return createModal({ id: 'offset-modal', icon: 'move-down-left', iconClass: 'text-blue-400',
    titleI18n: 'offsetTitle', titleDefault: 'Rzut wsteczny', content: `<div class="mb-4"><label class="block text-slate-300 text-sm mb-1"><span data-i18n="offsetDist">Dystans</span> (<span class="unit-label">m</span>)</label><input type="number" id="offset-dist" value="30" class="w-full bg-slate-700 text-white p-3 rounded-lg border border-slate-600 outline-none"></div>
...
function batteryModalHTML(): string {
  return createModal({ id: 'battery-modal', zIndex: 3000,
    borderClass: 'border-2 border-red-600', shadowClass: 'shadow-[0_0_30px_rgba(220,38,38,0.3)]',
    titleI18n: 'battLowTitle', titleDefault: 'Niski stan baterii!',
    titleColor: 'text-red-500', titleMargin: 'mb-2', icon: 'battery-warning',
    content: `<p class="text-slate-300 text-sm mb-6 leading-relaxed" data-i18n="battLowBody">Poziom naładowania spadł poniżej 15%. <strong>Podłącz ładowanie</strong>!</p>
...
function warningModalHTML(): string {
  return createModal({ id: 'warning-modal', titleI18n: 'infoTitle', titleDefault: 'Informacja',
    titleMargin: 'mb-2', icon: 'alert-triangle', iconClass: 'text-yellow-500',
    content: `<p class="text-slate-300 text-sm mb-6 leading-relaxed" id="warning-text"></p>
...
function wsSyncModalHTML(): string {
  return createModal({ id: 'ws-sync-modal', zIndex: 4000,
    borderClass: 'border border-blue-500', shadowClass: 'shadow-[0_0_15px_rgba(59,130,246,0.3)]',
    titleI18n: 'wsTitle', titleDefault: 'Zewnętrzny Monitor', titleMargin: 'mb-2',
    icon: 'smartphone', iconClass: 'text-blue-400', content: `<p class="text-slate-300 text-xs mb-4" data-i18n="wsDesc">Połącz się z telefonem z Androidem w koi przez lokalne Wi-Fi (Hotspot), aby działał jako zapasowy alarm.</p>
...
function apiKeyModalHTML(): string {
  return createModal({ id: 'api-key-modal', zIndex: 4000,
    borderClass: 'border border-purple-500', shadowClass: 'shadow-[0_0_15px_rgba(147,51,234,0.3)]',
    titleI18n: 'apiTitle', titleDefault: 'Klucz API', titleMargin: 'mb-2',
    icon: 'key', iconClass: 'text-purple-400', content: `<p class="text-slate-300 text-sm mb-3" data-i18n="apiDesc">Podaj darmowy klucz API od Google AI Studio.</p>
...
function aiModalHTML(): string {
  return createModal({ id: 'ai-modal', titleId: 'ai-modal-title', zIndex: 3000,
    innerPadding: 'p-5', borderClass: 'border border-purple-600',
    shadowClass: 'shadow-[0_0_15px_rgba(147,51,234,0.3)]',
    extraInnerClass: 'flex flex-col max-h-[90vh]', content: `<div class="flex justify-between items-start mb-2">
...
function aiSummaryModalHTML(): string {
  return createModal({ id: 'ai-summary-modal', zIndex: 3000,
    borderClass: 'border border-blue-500', shadowClass: 'shadow-[0_0_15px_rgba(59,130,246,0.2)]',
    titleI18n: 'logTitle', titleDefault: 'Dziennik Pokładowy',
    icon: 'book-open', iconClass: 'text-blue-400', content: `<div id="ai-summary-loader" class="flex flex-col items-center justify-center text-blue-400 py-6 gap-3"><i data-lucide="loader-2" class="animate-spin w-8 h-8"></i><span class="text-sm" data-i18n="logWriting">Pisanie...</span></div>
...
function gpsLostModalHTML(): string {
  return createModal({ id: 'gps-lost-modal', zIndex: 3500,
    borderClass: 'border-2 border-yellow-500', shadowClass: 'shadow-[0_0_25px_rgba(234,179,8,0.3)]',
    titleI18n: 'gpsLostTitle', titleDefault: 'Utrata sygnału GPS!',
    titleColor: 'text-yellow-500', titleMargin: 'mb-2', icon: 'satellite-dish',
    content: `<p class="text-slate-300 text-sm mb-2 leading-relaxed" data-i18n="gpsLostBody">Brak odczytu GPS przez ponad 60 sekund. Monitoring kotwicy jest zagrożony!</p>
...
function historyModalHTML(): string {
  return createModal({ id: 'history-modal', titleId: 'history-title', innerPadding: 'p-5',
    extraInnerClass: 'flex flex-col max-h-[90vh]', content: `<div class="flex justify-between items-center mb-4">
...
function statsModalHTML(): string {
  return createModal({ id: 'stats-modal', zIndex: 2500, innerPadding: 'p-5',
    borderClass: 'border border-cyan-500', shadowClass: 'shadow-[0_0_15px_rgba(6,182,212,0.2)]',
    titleI18n: 'statsTitle', titleDefault: 'Statystyki',
    icon: 'bar-chart-3', iconClass: 'text-cyan-400', content: `<div id="stats-content" class="space-y-2 text-sm">
...
function replayModalHTML(): string {
  return createModal({ id: 'replay-modal', zIndex: 2500, innerPadding: 'p-5',
    borderClass: 'border border-blue-500', shadowClass: 'shadow-[0_0_15px_rgba(59,130,246,0.2)]',
    extraInnerClass: 'flex flex-col max-h-[90vh]',
    titleI18n: 'replayTitle', titleDefault: 'Sesja', titleMargin: 'mb-2',
    icon: 'play-circle', iconClass: 'text-blue-400', content: `<div id="replay-info" class="text-slate-300 text-xs mb-3 space-y-1"></div>
...
function weatherModalHTML(): string {
  return createModal({ id: 'weather-modal', innerPadding: 'p-5', maxWidth: 'max-w-md',
    borderClass: 'border border-cyan-500', shadowClass: 'shadow-[0_0_15px_rgba(6,182,212,0.2)]',
    extraInnerClass: 'flex flex-col max-h-[90vh]',
    titleI18n: 'wxTitle', titleDefault: 'Prognoza Morska', titleMargin: 'mb-3',
    icon: 'cloud-sun', iconClass: 'text-cyan-400', content: `<div id="weather-loading" class="flex flex-col items-center justify-center py-8 gap-3">
...
function qrScanModalHTML(): string {
  return createModal({ id: 'qr-scan-modal', zIndex: 4000, innerPadding: 'p-5',
    borderClass: 'border border-emerald-500', shadowClass: 'shadow-[0_0_15px_rgba(16,185,129,0.3)]',
    titleI18n: 'qrTitle', titleDefault: 'Skanuj kod QR', titleMargin: 'mb-2',
    icon: 'qr-code', iconClass: 'text-emerald-400', content: `<p class="text-slate-400 text-xs mb-3" data-i18n="qrDesc">Zeskanuj kod QR wyświetlony na telefonie z Androidem, aby nawiązać połączenie.</p>
